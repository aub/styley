class PreviewImageController < ApplicationController
  def show
    ...
    system("nik2img.py -o #{RAILS_ROOT}/public/#{@output_path} -i png -p epsg:900913 --zoomto=#{params[:center_lon]},#{params[:center_lat]},#{params[:zoom_level]} -s 1024,512 -m #{xml_file.path} --noopen > /dev/null")
